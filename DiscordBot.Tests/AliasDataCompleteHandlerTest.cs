...
namespace DiscordBot.Tests
{
    public class AliasDataCompleteHandlerTest
    {
        ...
        {
            var data = new Dictionary<string, BSInfo>()
                {
                    { "Blunderbuss",
                        new BSInfo(){
            ...
            var db = new Mock<IDB>();
            db.Setup(d => d.TryGetTable(It.IsAny<string>(), out data))
                .Callback(new GetTable((string _, out Dictionary<string, BSInfo> d) => d = data)).Returns(true);
        ...
        private delegate void GetTable(string key, out Dictionary<string, BSInfo> data);
    }
}
